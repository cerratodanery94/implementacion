...
$consulta2=$conexion->prepare("SELECT * FROM tbl_personas p inner join tbl_expediente_nutricionista e on e.PER_CODIGO = p.PER_CODIGO where p.PER_CODIGO = $codigo_persona");


$consulta2->execute();
$products = $consulta2->fetchAll();
$pdf->setY(70);$pdf->setX(50);
$header = array("Peso","Altura");
//// Arrar de Productos
    ...
    {
        $pdf->setY(77);$pdf->setX(50);
        $pdf->Cell($w[1],4,$row['NUTRI_PESO'],1);
        $pdf->Cell($w[1],4,$row['NUTRI_ALTURA'],1);
    ...
    //// Arrar de Productos
        ...
        foreach($products as $row)
        {
            $pdf->setY(97);$pdf->setX(50);
            $pdf->MultiCell(100,5,$row['NUTRI_DIETA'],1,'C');
            $pdf->Ln();
        ...
        //// Arrar de Productos
            ...
            foreach($products as $row)
            {
                $pdf->setY(127);$pdf->setX(50);
                $pdf->MultiCell(100,5,$row['NUTRI_ANTECEDENTES_CLINICOS'],1,'C');
                $pdf->Ln();
