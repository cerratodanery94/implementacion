...
$resultado2->execute(array(":id"=>NULL,":usuc"=>$_SESSION["id_us"],":objeto"=>32,":accion"=>'INGRESO',":descr"=>'INGRESO ALA PANTALLA EDITAR ROLES',":fecha"=>date("Y-m-d H:i:s")));
  if(isset($_GET['id'])){
    $id=$_GET['id'];
    $sql="SELECT * FROM TBL_ROL WHERE ROL_CODIGO= :id";
  $resultado=$conexion->prepare($sql);	
    $resultado->execute(array(":id"=>$id));
   if ($resultado->rowCount()>=1) {
       $fila=$resultado->fetch();
       $id_r=$fila['ROL_CODIGO'];
       $rol=$fila['ROL_NOMBRE'];
       $descrip=$fila['ROL_DESCRIPCION'];
...
<?php require "../modelos/actualizar_rol_modelo.php"?>
