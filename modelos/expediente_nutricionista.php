...
if ( $_GET["id"]) {
$id=$_GET["id"];
$consulta=$conexion->prepare("SELECT * FROM tbl_personas p inner join tbl_paises s on p.PAIS_CODIGO=s.PAIS_CODIGO inner join tbl_expediente_nutricionista e on e.PER_CODIGO = p.PER_CODIGO  where P.PER_CODIGO=$id");
$consulta->execute();
...
$datos=$consulta->fetch(); 
    ...
    $this->Cell(0,0,utf8_decode('FICHA NUTRICIONISTA'),0,0,'C');
...
$consulta2=$conexion->prepare("SELECT * FROM tbl_personas p inner join tbl_paises s on p.PAIS_CODIGO=s.PAIS_CODIGO inner join tbl_expediente_nutricionista e on e.PER_CODIGO = p.PER_CODIGO  where P.PER_CODIGO=$id");
$consulta2->execute();
while($fila=$consulta2->fetch()){
    ...
    $pdf->Cell(150,5,utf8_decode("DATOS CLÍNICOS"),1,1,'C',1);
    $pdf->setY(85);$pdf->setX(25);
    $pdf->SetFont('Arial','',10);
    $pdf->SetTextColor(0,0,0);
    $pdf->Cell(150,7,utf8_decode('PESO: '.$fila['NUTRI_PESO'].' LIBRAS'),0,1,'J');
    $pdf->setY(90);$pdf->setX(25);
    $pdf->Cell(155,7,utf8_decode('ESTATURA: '.$fila['NUTRI_ALTURA'].' METROS'),0,'J');
    $pdf->Ln();
    $pdf->setY(110);$pdf->setX(25);
    $pdf->SetFont('Arial','B',10);
    $pdf->SetFillColor(45,65,84);
    $pdf->SetTextColor(255,255,255);
    $pdf->Cell(150,5,utf8_decode("DIETA"),1,1,'C',1);
    $pdf->setY(115);$pdf->setX(25);
    $pdf->SetFont('Arial','',10);
    $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(150,7,utf8_decode($fila['NUTRI_DIETA']),1,'J');
    $pdf->Ln();
    $pdf->setY(160);$pdf->setX(25);
    $pdf->SetFont('Arial','B',10);
    $pdf->SetFillColor(45,65,84);
    $pdf->SetTextColor(255,255,255);
    $pdf->Cell(150,5,utf8_decode("ANTECEDENTES CLÍNICOS"),1,1,'C',1);
    $pdf->setY(165);$pdf->setX(25);
    $pdf->SetFont('Arial','',10);
    $pdf->SetTextColor(0,0,0);
    $pdf->MultiCell(150,7,utf8_decode($fila['NUTRI_ANTECEDENTES_CLINICOS']),1,'J');
    $pdf->Ln();
