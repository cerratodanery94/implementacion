...
$resultado2->execute(array(":id"=>NULL,":usuc"=>$_SESSION["id_us"],":objeto"=>32,":accion"=>'INGRESO',":descr"=>'INGRESO ALA PANTALLA EDITAR ROLES',":fecha"=>date("Y-m-d"),":hora"=>date("H:i:s")));
  if(isset($_GET['id'])){
    $id=$_GET['id'];
    $sql="SELECT * FROM TBL_OBJETOS WHERE OBJ_CODIGO= :id";
  $resultado=$conexion->prepare($sql);	
    $resultado->execute(array(":id"=>$id));
   if ($resultado->rowCount()>=1) {
       $fila=$resultado->fetch();
       $id=$fila['OBJ_CODIGO'];
       $nombre=$fila['OBJ_NOMBRE'];
       $descripcion=$fila['OBJ_DESCRIPCION'];
       $tipo=$fila['OBJ_TIPO'];
...
<header class="main-header">
  ...
  <div class="content-wrapper">
    ...
    Editar descripción de pantallas
        <small>ClimeHome</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="../vistas/mostrar_pantallas.php"><i class="fa fa-television"></i>Pantallas</a></li>
        <li class="active"><i class="fa fa-list-alt"></i> Editar descripción de pantallas</li>
      </ol>
    </section>
...
<?php require "../modelos/actualizar_pantallas_modelo.php"?>
